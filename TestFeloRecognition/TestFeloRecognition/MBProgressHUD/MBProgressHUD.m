...
- (void)layoutSubviews {
	...
	// Determine the total widtd and height needed
	CGFloat maxWidth = bounds.size.width - 6 * margin;
	CGSize totalSize = CGSizeZero;
	
    CGRect indicatorF = indicator.bounds;
    indicatorF.size.width = MIN(indicatorF.size.width, maxWidth);
    if (!CGRectIsEmpty(indicatorF)) {
        indicatorF.size.width = MAX(indicatorF.size.width, 37);
        indicatorF.size.height = MAX(indicatorF.size.height, 37);
    }
	totalSize.width = MAX(totalSize.width, indicatorF.size.width);
	totalSize.height += indicatorF.size.height;
	
    CGFloat maxHeight = bounds.size.height - totalSize.height - kPadding - 4 * margin;
    CGSize labelSize = MB_MULTILINE_TEXTSIZE(label.text, label.font, CGSizeMake(maxWidth, maxHeight), label.lineBreakMode);
